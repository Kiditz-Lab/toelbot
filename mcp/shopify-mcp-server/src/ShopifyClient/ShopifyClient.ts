  ...
  async createProduct(
    ...
    const variables = {
      input: {
        ...
        descriptionHtml: productInput.description,
    ...
    try {
      const response = await this.shopifyGraphqlRequest<{
        productCreate: {
          product: ProductNode;
          userErrors: Array<{
            field: string[];
      ...
      if (response.data.productCreate.userErrors.length > 0) {
        throw getGraphqlShopifyUserError(response.data.productCreate.userErrors);
      }
